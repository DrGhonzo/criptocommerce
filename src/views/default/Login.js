...
const Login = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/v01/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Guarda el token en el almacenamiento local (por ejemplo, localStorage o sessionStorage)
        localStorage.setItem('token', data.token);
        setLoggedIn(true);
        // Realiza cualquier redirección o acción necesaria después de iniciar sesión
